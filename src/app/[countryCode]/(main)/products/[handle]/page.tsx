...
export default async function ProductPage(props: Props) {
    ...
    if (!testimonialsContent) {
        testimonialsContent =
            await getTestimonialsSectionByKey("pdp-testimonials");
    }
        ...
        try {
            ...
            if (!ritualProd) {
                ...
                try {
                    ...
                    const allProducts =
                        allProductsResponse.response.products || [];
                    ...
                    const possibleMatches = allProducts.filter((prod) => {
                        ...
                        const metadataMatch =
                            (
                                prod.metadata?.["product-name"] as string
                            )?.toLowerCase() ===
                    ...
                    if (variant) {
                        ...
                        ritualProduct = {
                            ...
                            currency:
                                ...
                                "inr",
                            image:
                                ritualProd.thumbnail ||
                                ritualProd.images?.[0]?.url,
                        };
    ...
    let afterlifeContent = await getAfterlifeSectionByProductHandle(
        params.handle,
    );
    ...
    const hasGiftOption =
        pricedProduct?.categories?.some((category: any) => {
            const categoryMetadata = category?.metadata as
                | Record<string, any>
                | undefined;
            return (
