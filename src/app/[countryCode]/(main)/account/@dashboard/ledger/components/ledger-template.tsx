...
export default function LedgerTemplate() {
    ...
    const findProductHandle = async (productName: string): Promise<string | null> => {
        try {
            ...
            const matchingProduct = allProducts.find((prod) => {
                ...
                const titleMatch =
                    titleWords.length > 0 &&
                    titleWords.some((word) => prodTitle.includes(word) || prodHandle.includes(word))

                // Also check if search name contains product title words
                const prodTitleWords = prodTitle.split(/\s+/).filter((w) => w.length > 2)
                const reverseMatch =
                    prodTitleWords.length > 0 &&
                    prodTitleWords.some((word) => normalizedSearchName.includes(word))

                // Direct match
                const directMatch =
                    prodTitle.includes(normalizedSearchName) ||
    ...
    const findProductVariantId = async (
        productName: string,
        itemSize?: string
    ): Promise<string | null> => {
        try {
            ...
            if (matchingProduct && matchingProduct.variants && matchingProduct.variants.length > 0) {
                ...
                if (itemSize) {
                    ...
                    const sizeVariant = matchingProduct.variants.find((v: any) => {
                        ...
                        return (
                            variantTitle.includes(sizeLower) ||
    ...
    const highlightBotanicalTerms = (text: string): React.ReactNode => {
        ...
        return parts.map((part, index) => {
            ...
            if (isTerm) {
                return (
                    <span key={index} className="font-medium text-black/60">
                        {part}
                    </span>
                )
            }
    ...
    return (
        ...
        >
            ...
            {ledger.length === 0 ? (
                ...
                >
                    <p
                        className="font-din-arabic text-sm sm:text-base text-black/60 mb-10 max-w-md mx-auto"
                        ...
                        <div className="mb-10 pb-7 border-b-2" style={{ borderColor: "#8b7759" }}>
                            <div className="grid grid-cols-12 gap-6 items-center">
                                <div className="col-span-1 flex items-center">
                                    <p
                                        className="font-american-typewriter text-xs tracking-wider"
                                        style={{ color: "#6b5d3f" }}
                                    >
                                        No.
                                    </p>
                                ...
                                <div
                                    className="col-span-11 md:col-span-5 flex items-center justify-center"
                                ...
                                {ledger.map((item: LedgerItem, index: number) => (
                                    ...
                                    >
                                        ...
                                        >
                                            ...
                                            <div className="col-span-11 md:col-span-5">
                                                {productHandles[item.id] ? (
                                                    ...
                                                    >
                                                        {/* Thumbnail */}
                                                        <div
                                                            className="relative w-32 h-32 flex-shrink-0 rounded-sm overflow-hidden border z-10"
                                                        ...
                                                        <div className="flex-1 min-w-0 relative z-10 pt-1">
                                                            <h3
                                                                className="font-american-typewriter mb-3 text-sm sm:text-base leading-snug text-black/60 transition-colors duration-300 group-hover:text-black"
                                                            ...
                                                            {(item.batchNo || item.origin || item.primaryNote) && (
                                                                <p
                                                                    className="font-din-arabic text-xs sm:text-sm text-black/40 mb-2 leading-relaxed"
                                                                ...
                                                                >
                                                                    ...
                                                                    {item.primaryNote && (
                                                                        <>
                                                                            <span>
                                                                                {" "}
                                                                                · Primary {item.category === "candle" ? "Note" : "Extract"}:{" "}
                                                                            </span>
                                                                            <span className="transition-colors duration-300 group-hover:text-black">
                                                                                {item.primaryNote}
                                                                            </span>
                                                            ...
                                                            {item.description && (
                                                                <p
                                                                    className="font-din-arabic text-xs sm:text-sm text-black/40 line-clamp-2 leading-relaxed"
                                                                    style={{ letterSpacing: "0.05em" }}
                                                                >
                                            ...
                                            {/* Value - Desktop */}
                                            <div
                                                className="hidden md:flex md:col-span-3 items-start justify-center pt-1"
                                                style={{ marginLeft: "40px" }}
                                            >
                                            ...
                                            {/* Actions - Desktop */}
                                            <div
                                                className="hidden md:flex md:col-span-3 items-center justify-center pt-1"
                                                style={{ marginLeft: "-20px" }}
                                            >
                        ...
                        <div className="mt-8 pt-6 border-t-2" style={{ borderColor: "#8b7759" }}>
                            ...
                            <div className="grid grid-cols-12 gap-6 mb-4">
                                ...
                                <div className="col-span-12 md:col-span-5 flex items-center px-4 md:px-0">
                                    <p
                                        className="font-american-typewriter text-xs sm:text-sm"
                                        style={{ letterSpacing: "0.05em", color: "#2d2520" }}
                                    >
                                        Total Recorded Value
                                    </p>
                                    <div
                                        className="flex-1 mx-2 md:mx-3 border-b"
                                        style={{ borderColor: "#8b7759", borderStyle: "dotted" }}
                                    ></div>
                                </div>
                                <div className="col-span-12 md:col-span-3 flex items-center justify-center px-4 md:px-0 -mt-3 md:mt-0 md:ml-10">
                                    <p
                                        className="font-din-arabic text-base sm:text-lg tabular-nums"
                                        style={{ color: "#8b7759", letterSpacing: "0.05em" }}
                                    >
                                        ₹
                                        {ledger
                                            .reduce((sum: number, item: LedgerItem) => sum + item.price, 0)
                                            .toLocaleString()}
                                    </p>
                                </div>
