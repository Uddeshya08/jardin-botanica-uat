...
const buildSizeOptions = (
    ...
    return (product.variants || [])
        .map((variant) => {
            const label = sizeOptionId
                ? (variant?.options?.find(
                      (opt) => opt.option_id === sizeOptionId,
                  )?.value ??
                  variant?.title ??
                  "")
        ...
        .sort(
            (a, b) => extractNumericSize(a.label) - extractNumericSize(b.label),
        );
};
...
export function ProductHero({
    ...
    const handleToggleLedger = () => {
        ...
        toast.success(
            `${product.title} ${alreadyInLedger ? "Removed From" : "Added To"} Ledger`,
            {
                duration: 2000,
            },
        );
                        ...
                        >
                            ...
                            <RadioGroup
                                ...
                                onValueChange={(value: string) => {
                                    console.log(
                                        "🔍 RadioGroup - onValueChange called:",
                                        {
                                            newValue: value,
                                            currentSelected: selectedVariantId,
                                            visibleOptions:
                                                visibleSizeOptions.map((o) => ({
                                                    id: o.id,
                                                    label: o.label,
                                                })),
                                        },
                    ...
                    >
                        ...
                        <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3 sm:gap-4">
                            ...
                            <motion.button
                                ...
                                disabled={
                                    !selectedVariantId || adding || isPending
                                }
                            >
